...
cs_setup_etc_hosts() {
	...
	sed -i -e "/^[[:space:]]*${priv_ip}/s:\(.*${priv_ip}\).*:\1 ${HOSTNAME} ${addrstr}:" /etc/hosts
    else
	echo "${priv_ip} ${HOSTNAME} ${addstr}" >> /etc/hosts
    fi
    addstr="# Added for condor to specify central manager of local network"
    if grep -q "${CM_HOST_IP}" /etc/hosts ; then
	sed -i -e "s:.*\(${addr}\):${CM_HOST_IP} ${CM_HOST_NAME} ${addstr}:" /etc/hosts
    else
